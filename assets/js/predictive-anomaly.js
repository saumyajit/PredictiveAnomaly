 ...
 *
 * IMPORTANT: Zabbix injects module assets in <head> BEFORE the view template
 * renders window.PAD_CONFIG at the bottom of <body>. All logic is therefore
 * deferred inside DOMContentLoaded so PAD_CONFIG is guaranteed to exist.
 */
...
// Safety guard — if view didn't render PAD_CONFIG, stop cleanly
if (!CFG || !ROOT) {
	console.error('PAD: PAD_CONFIG or #pad-root not found');
	return;
}
...
async function apiFetch(action, params = {}) {
	...
	try {
		...
		console.error('PAD: non-JSON response from', action, text.slice(0, 400));
...
qs('#pad-theme-btn') && qs('#pad-theme-btn').addEventListener('click', () =>
...
qsa('.pad-tab').forEach(btn => {
	btn.addEventListener('click', function () {
		...
		const panel = qs(`#tab-${state.active_tab}`);
...
if (scoreRange) {
	scoreRange.addEventListener('input', () => {
		const v = qs('#pad-score-val');
		if (v) v.textContent = parseFloat(scoreRange.value).toFixed(2);
	});
}
...
(function () {
	...
	function addTag(id, name) {
		...
		tag.innerHTML = `${escHtml(name)}<button type="button" class="pad-ms-tag-remove" data-id="${id}">×</button>`;
...
if (sortFieldEl) {
	sortFieldEl.addEventListener('change', function () {
...
qs('#pad-prev-page') && qs('#pad-prev-page').addEventListener('click', () => {
	if (state.page > 1) { state.page--; loadGroupData(); }
});
qs('#pad-next-page') && qs('#pad-next-page').addEventListener('click', () => {
...
function renderSummary(summary, total_groups) {
	...
	function set(id, val, sub) {
		...
		const v  = el.querySelector('.pad-stat__value');
		const s  = el.querySelector('.pad-stat__sub');
...
function renderGroupTable(groups) {
	...
	tbody.innerHTML = '';
	groups.forEach(g => {
		const sc  = scoreColor(g.anomaly_score || 0);
		const pct = Math.round((g.anomaly_score || 0) * 100);
		...
		tr.innerHTML = `
			...
			<td><div class="pad-score-cell">
				...
				<span class="pad-score-num" style="color:${sc}">${(g.anomaly_score||0).toFixed(2)}</span>
			</div></td>
	...
	qsa('.pad-drill-btn', tbody).forEach(b =>
		b.addEventListener('click', () => openDrilldown(b.dataset.groupid, b.dataset.groupname))
	);
}
...
function renderPagination(page, total_pages, total) {
	const info = qs('#pad-page-info');
	if (info) info.textContent = `Page ${page} of ${total_pages} (${total || 0} groups)`;
...
function openDrilldown(groupid, groupname) {
	...
	const title   = qs('#pad-drawer-title');
	const sub     = qs('#pad-drawer-sub');
	const body    = qs('#pad-drawer-body');
	const overlay = qs('#pad-drawer-overlay');
	if (title)   title.textContent = groupname;
	if (sub)     sub.textContent   = CFG.strings.loading;
...
qs('#pad-drawer-overlay') && qs('#pad-drawer-overlay').addEventListener('click', e => {
...
async function fetchHostData(groupid, groupname) {
	try {
		...
		const body = qs('#pad-drawer-body');
		if (body) body.innerHTML = `<div class="pad-error">⚠ ${escHtml(e.message)}</div>`;
	}
}
...
function renderDrilldown(data, groupname) {
	...
	[
		{ l:'Total Hosts', v:(data.total_hosts||0).toLocaleString(),        c:'#2563eb' },
		{ l:'Anomalous',   v:(data.hosts||[]).length,                        c:'#ef4444' },
		{ l:'Page',        v:`${data.page||1}/${data.total_pages||1}`,       c:'#06b6d4' },
	].forEach(s => {
	...
	if (data.hosts && data.hosts.length) {
		...
		const cpuItem = first.metrics && first.metrics.cpu;
		if (cpuItem && cpuItem.itemid) {
			...
			card.innerHTML = `
				...
				<div class="pad-card__body">
					<div class="pad-chart-wrap" style="height:180px"><canvas id="drilldown-cpu-chart"></canvas></div>
				</div>`;
	...
	tableCard.innerHTML = `
		<div class="pad-card__head">
			<div class="pad-card__title">🖥 ${escHtml(groupname)}</div>
			<div class="pad-card__actions"><button class="pad-btn pad-btn--sm">All ${(data.total_hosts||0).toLocaleString()}</button></div>
		</div>
		<div class="pad-table-wrap">
			<table class="pad-table">
				<thead><tr><th>Host</th><th>Score</th><th>CPU</th><th>Memory</th><th>Disk</th><th>Breach ETA</th><th>Action</th></tr></thead>
				<tbody>${rows}</tbody>
			</table>
		</div>`;
...
async function loadForecastChart(canvasId, itemid) {
	try {
		const url = `zabbix.php?action=${CFG.action_forecast}&itemid=${itemid}&time_range=${CFG.filter.time_range||'24h'}&model=${CFG.filter.model||'all'}`;
		const r   = await fetch(url, { headers: { 'X-Requested-With': 'XMLHttpRequest' } });
...
function renderForecastOnCanvas(canvasId, data) {
	...
	const ptC     = (data.series   || []).map(p => p.anomaly ? '#ef4444' : 'transparent');
	const ptR     = (data.series   || []).map(p => p.anomaly ? 5 : 0);
	const N       = aLabels.length;
	const full_a  = [...aVals, ...new Array(fLabels.length).fill(null)];
	const full_f  = [...new Array(N - 1).fill(null), aVals[N-1], ...fVals];
	const full_u  = [...new Array(N).fill(null), ...upper];
	const full_l  = [...new Array(N).fill(null), ...lower];
	state.charts[canvasId] = new Chart(canvas.getContext('2d'), {
		type: 'line',
		data: { labels: [...aLabels, ...fLabels], datasets: [
			...
			{ label:'Actual',   data:full_a, borderColor:'#2563eb', borderWidth:2, backgroundColor:'transparent', pointBackgroundColor:ptC, pointRadius:ptR, tension:0.4 },
		]},
		options: { responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{
			x:{ grid:{ color:c.grid }, ticks:{ color:c.tick, maxTicksLimit:8 } },
			y:{ grid:{ color:c.grid }, ticks:{ color:c.tick, callback: v => v+(data.units||'%') } },
		}},
	});
}
...
function buildFleetLine(id, actualColor, forecastColor, units, opts) {
	...
	state.charts[id] = new Chart(canvas.getContext('2d'), {
		...
		options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{
			x:{ grid:{ color:c.grid }, ticks:{ color:c.tick, maxTicksLimit:9 } },
			y:{ min:0, max:100, grid:{ color:c.grid }, ticks:{ color:c.tick, callback:v=>v+units } },
		}},
	});
}
...
function buildDiskChart() {
	...
	const hosts  = [
		{ l:'Database Servers', c:'#2563eb', b:55, s:0.55 },
		{ l:'Storage Nodes',    c:'#ef4444', b:72, s:0.70 },
		{ l:'Web Servers',      c:'#10b981', b:42, s:0.35 },
	];
	const datasets = hosts.map(h => {
		const actual   = Array.from({length:N},(_,i) => h.b+h.s*i+(Math.random()-.5)*2);
		const forecast = Array.from({length:F},(_,i) => actual[N-1]+h.s*(i+1));
		return { label:h.l, data:[...actual,...forecast], borderColor:h.c, borderWidth:2, backgroundColor:'transparent', pointRadius:0, tension:0.3,
			segment:{ borderDash: ctx => ctx.p0DataIndex>=N-1 ? [5,3] : [] } };
	});
	...
	state.charts['chart-fleet-disk'] = new Chart(canvas.getContext('2d'), {
		...
		options:{ responsive:true, maintainAspectRatio:false, plugins:{ legend:{ display:false } }, scales:{
			...
			y:{ min:20, max:100, grid:{ color:c.grid }, ticks:{ color:c.tick, callback:v=>v+'%' } },
		}},
	});
}
...
function renderHeatmap() {
	...
	if (!state.groups.length) {
		wrap.innerHTML = `<div class="pad-empty">Load Group Overview first.</div>`; return;
	}
	...
	if (scale) { scale.innerHTML=''; [0.05,0.2,0.35,0.5,0.65,0.8,0.95].forEach(v=>{ const c=el('div','pad-hm-scale-cell'); c.style.background=hmColor(v); scale.appendChild(c); }); }
}
...
function renderExhaustion() {
	...
	if (!state.groups.length) {
		if(body) body.innerHTML=`<div class="pad-empty">No data — load Group Overview first.</div>`;
		if(mw)   mw.innerHTML  =`<div class="pad-empty">—</div>`; return;
	}
	if (body) {
		body.innerHTML='';
		[...state.groups].sort((a,b)=>(b.disk||0)-(a.disk||0)).slice(0,8).forEach(g=>{
			const c=usageColor(g.disk||0),div=el('div','pad-ex-item');
			div.innerHTML=`<div><div class="pad-ex-host">${escHtml(g.worst_host||g.name.split(' ')[0])}</div><div class="pad-ex-meta">${escHtml(g.name)}</div></div><div><div class="pad-ex-track"><div class="pad-ex-fill" style="width:${Math.min(100,g.disk||0)}%;background:${c}88"></div></div><div class="pad-ex-labels"><span>${(g.disk||0).toFixed(1)}% now</span><span>⚡ 85% limit</span></div></div><div class="pad-ex-eta" style="color:${c}">${(g.disk||0)>80?'< 7d':(g.disk||0)>65?'~30d':'> 30d'}</div>`;
	...
	if (mw) {
		...
		urgent.forEach((g,i)=>{
			...
			div.innerHTML=`<div class="pad-mw-sev" style="background:${c}"></div><div class="pad-mw-countdown"><span class="pad-mw-days" style="color:${c}">${days}</span><span class="pad-mw-dlabel">${days===0?'TODAY':'DAYS'}</span></div><div class="pad-mw-body"><div class="pad-mw-title">${escHtml(g.worst_host||g.name)} — Action Required</div><div class="pad-mw-sub">${escHtml(g.name)} · Score ${(g.anomaly_score||0).toFixed(2)}</div></div>`;
			mw.appendChild(div);
		});
	}
}
...
function renderModels() {
	const body=qs('#pad-model-body'); if(!body) return;
	body.innerHTML='';
	const models=[
		{dot:'on',  name:'Zabbix Native Trends · Linear Regression',info:`${CFG.total_hosts.toLocaleString()} hosts · CPU/Disk/Mem · 30d window · Primary`,acc:'93.4%',cls:'hi'},
		...
		{dot:'idle',name:'Isolation Forest · Multivariate',         info:'Scheduled retrain in 3h · n_estimators=200 · Top 500 hosts',acc:'86.5%',cls:'med'},
