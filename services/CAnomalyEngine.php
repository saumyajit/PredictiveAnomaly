 ...
 * Z-score anomaly detection + OLS linear regression forecasting.
 */
...
class CAnomalyEngine {
	...
	public function zScoreAnomalyScore(array $values): array {
		...
		for ($i = 0; $i < $n; $i++) {
			...
			$z_scores[] = round($z, 3);
			if ($z > self::Z_THRESHOLD) $anomaly_indices[] = $i;
			$max_z = max($max_z, $z);
		}
	...
	public function linearRegressionForecast(array $clocks, array $values, string $time_range = '24h'): array {
		...
		for ($i = 1; $i <= self::FORECAST_STEPS; $i++) {
		...
		$breach_eta = $this->estimateBreachEta($a, $b, $t_norm, 85.0);
		$slope_score = 0;
	...
	private function estimateBreachEta(float $a, float $b, array $t_norm, float $threshold): ?int {
		...
		$eta = (int)(($threshold - $a) / $b - end($t_norm));
	...
	private function forecastStepSize(string $time_range, array $clocks): int {
		...
		return match($time_range) {
			'1h'  => 300, '6h' => 900, '24h' => 3600,
			'7d'  => 21600, '30d' => 86400, default => 3600,
		};
	}
